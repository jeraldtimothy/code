...
function savings(grossPay, taxRate, expenses) {
    const afterTaxPay = Math.floor(grossPay - (grossPay * taxRate));
    const remainingPay = afterTaxPay - expenses;
    return remainingPay;
}
...
function interest(principal, rate, periods) {
	const sampleinterest = principal * (rate * periods);
    const finalvalue = Math.floor(sampleinterest + principal);
    return finalvalue;
}
